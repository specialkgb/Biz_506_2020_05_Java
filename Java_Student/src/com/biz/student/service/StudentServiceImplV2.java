...
// 필드변수와 method는 public, protected로 되어 있는 경우만 상속된다.
...
public class StudentServiceImplV2 extends StudentServiceImplV1 {
	...
	// V1에서 구현된 studentList() method를 다시 새롭게 구현하기 위해 method 생성
	// List를 Console에 보여주는 방식이었는데
	// List결과는 file에 저장하여 프로젝트가 종료되어도 언제든지
	// 결과를 확인할 수 있도록 구현해보자
	// 데이터, 결과를 파일로 저장하는 것은 결과를 영구 보관하는 목적이 있다.
	// 이러한 것을 project에서는 persistence라고 한다.
	@Override
	public void studentList() {
		...
		for (Student student : studentList) {
			...
			outPut.print(student.getNum() + "\t");
			outPut.print(student.getName() + "\t");
			outPut.print(student.getGrade() + "\t");
			outPut.println(student.getDepth());
		}
		outPut.println("==================================");
		outPut.close();
	}
