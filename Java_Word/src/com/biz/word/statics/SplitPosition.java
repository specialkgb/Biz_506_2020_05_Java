...
public class SplitPosition {
	...
	// static으로 선언된 변수는 누구나
	// 어디에서든지 접근하여 읽기, 쓰기 가능하다
	// 만약 WORD_ENG 변수를 어딘가에서 0이 아닌 다른 값으로
	// 변경을 해버리면 WORD_ENG변수를 진짜 사용해야 할 곳에서 문제가 발생할 수 있다.
	// 그래서 이러한 변수에는 final 키워드를 붙여준다.
	// final 키워드가 부착된 변수는 다른 곳에서는 읽기만 가능하고
	// 절대 값을 변경할 수 없다.
